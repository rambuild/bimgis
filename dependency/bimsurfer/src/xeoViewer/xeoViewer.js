define([
	...
	function xeoViewer(cfg) {
		...
		var scene = (self.scene = new xeogl.Scene({
			// http://xeoengine.org/docs/classes/Scene.html
			canvas: canvas,
			transparent: true
		}))
		...
		var cameraFlight = new xeogl.CameraFlightAnimation(scene, {
			// http://xeoengine.org/docs/classes/CameraFlightAnimation.html
			fitFOV: 25,
			duration: 1
		})
		...
		cameraControl.on("nopick", function (hit) {
			self.setSelection({
				clear: true
			})
		})
		...
		this.createGeometry = function (geometryId, positions, normals, colors, indices) {
			var geometry = new xeogl.Geometry(scene, {
				// http://xeoengine.org/docs/classes/Geometry.html
		...
		this.clear = function () {
			...
			collection.iterate(function (component) {
				list.push(component)
			})
		...
		// ****************** 自定义恢复配置 ******************
		this.resetByConf = function (bookmark, params) {
			if (!bookmark) {
				return console.log("无配置参数输入")
			}
			this.setBookmark(bookmark, params)
		}
