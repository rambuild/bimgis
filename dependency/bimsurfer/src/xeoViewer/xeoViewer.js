    ...
    function xeoViewer(cfg) {
        ...
        this.setBookmark = function (bookmark, options) {
            ...
            if (setVisible) {
                ...
                // ************** 自定义添加，先删除所有的构件 *************
                let allAttrs = [
                    'IfcSlab', 'IfcWall', 'IfcWallStandardCase', 'IfcFurnishingElement', 'IfcBeam',
                    'IfcBuildingElementProxy', 'IfcColumn', 'IfcDoor', 'IfcFlowTerminal', 'IfcOpeningElement',
                    'IfcRailing', 'IfcRamp', 'IfcSpace', 'IfcStair', 'IfcTransportElement', 'IfcWindow']
                // 恢复配置前将所有构件隐藏
                this.setVisibility({
                    types: allAttrs,
                    visible: false
