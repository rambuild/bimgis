; (function (global) {
	class MyLeg {
		...
		getCheckedNodes() { //获取选中id集合
			let arr = []
			$.each($('#legTreeBox input:checkbox:checked'), function () {
				let temp = $(this).val();
		...
		getHalfSelected() { //半选中id集合
			let arr = []
			$.each($('#legTreeBox  .iconjianhao'), function () {
		...
		synTree(options) {
			...
			//		$(options.ele).on('click',"ul li",function(e){//点击li标签
			//			if($(this).children('.iconxiangyou').attr("data-isParent") == "true"){
			//				$.get(options.url,{id:$(this).parent().attr("id")},res=>{
			//					$(this).append(that.creaTree(res));
			//				})
			//				$(this).children('.iconxiangyou').attr("data-isParent",'0')
			//				$($(this).children('.iconxiangyou')).attr("style", 'transform:rotate(90deg)');
			//			}else if($(this).children('.iconxiangyou').attr("data-isParent") == 0){
			//				$(this).children('ul').attr("style", 'display: none;');
			//				$(this).children('.iconxiangyou').attr("data-isParent",'1')
			//				$($(this).children('.iconxiangyou')).attr("style", 'transform:rotate(0deg)');
			//			}else if($(this).children('.iconxiangyou').attr("data-isParent") == 1){
			...
			//			e.stopPropagation();
			//		})
			$(options.ele).on('click', "#legTreeBox ul li .iconxiangyou", function (e) {//点击向下图标
		...
		tree(options) {
			...
			function createTree(data) {
				...
				for (let i = 0; i < data.length; i++) {
					...
					str += '</label><i class="iconfont ' + data[i].ico + '"/><a>' + data[i].name + '</a>';
			...
			//是否展开
			$.each($("#legTreeBox input"), function () {
